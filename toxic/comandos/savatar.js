const {
    MessageEmbed
} = require("discord.js");
const config = require("../config.json");
module.exports.run = async (client, message, args) => {
    message.delete({
        "timeout": 300,
        "reason": "kkj"
    });
    let color = config.cor;
    let prem = config.perm;
    if (!prem.includes(message.author.id))
        return message.reply("Você não tem permissão!");
    const QcGE = args.join(" ");
    if (!QcGE)
        return message.reply("Coloque o link do avatar!").then(msg => msg.delete({
            "timeout": 8000,
            "reason": "kkj"
        }));
    const gVqE = new MessageEmbed()
    .setAuthor("- toxic divulgações", client.user.avatarURL({
        "dynamic": true,
        "format": "png",
        "size": 1024
    }), "https://discord.gg/bBMmZ6n")
    .setDescription(`> Avatar setado para:`)
    .setImage(QcGE)
    .setColor(color);
    client.user.setAvatar(QcGE).then(IWtE => message.channel.send(gVqE)).catch(IqhF => message.reply(`Erro ao setar avatar!`));
};